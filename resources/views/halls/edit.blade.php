<x-base-layout>
    <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md mt-6">
        <h1 class="text-3xl font-bold mb-8">Zaal Bewerken</h1>

        <form action="{{ route('halls.update', $hall->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <!-- Naam -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700">
                    Zaalnaam
                </label>
                <input type="text" name="name" id="name" required
                       value="{{ old('name', $hall->name) }}"
                       class="mt-1 block w-full border border-gray-300 rounded-md p-2 bg-gray-100 focus:ring-indigo-500 focus:border-indigo-500">
            </div>

            <!-- Capaciteit -->
            <div>
                <label for="capacity" class="block text-sm font-medium text-gray-700">
                    Capaciteit (stoelen)
                </label>
                <input type="number" name="capacity" id="capacity" min="1" required
                       value="{{ old('capacity', $hall->capacity) }}"
                       class="mt-1 block w-full border border-gray-300 rounded-md p-2 bg-gray-100 focus:ring-indigo-500 focus:border-indigo-500">
            </div>

            <!-- Schermgrootte -->
            <div>
                <label for="screen_size" class="block text-sm font-medium text-gray-700">
                    Schermgrootte
                </label>
                <input type="text" name="screen_size" id="screen_size" required
                       value="{{ old('screen_size', $hall->screen_size) }}"
                       class="mt-1 block w-full border border-gray-300 rounded-md p-2 bg-gray-100 focus:ring-indigo-500 focus:border-indigo-500">
            </div>

            <!-- Knoppen -->
            <div class="flex space-x-4">
                <a href="{{ route('halls.index') }}"
                   class="mt-3 px-4 py-3 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition text-center">
                    Terug
                </a>

                <button type="submit"
                        class="mt-3 px-4 py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">
                    Opslaan
                </button>
            </div>
        </form>
    </div>

    <!-- Succesmelding -->
    @if (session('success'))
        <div class="mb-4 flex items-center justify-between rounded bg-green-100 px-4 py-3 text-green-800">
            <span>{{ session('success') }}</span>
            <button onclick="this.parentElement.remove()" class="ml-4 font-bold text-green-800">&times;</button>
        </div>
    @endif

            @if ($errors->any())
        <div class="alert alert-danger mt-4">
            <ul>
                @foreach ($errors->all() as $error)
                    <li class="text-red-600">{{ $error }}</li>
                @endforeach
            </ul>
        </div>
        @endif
</x-base-layout>
